...
public class BatterPanel extends JPanel implements ActionListener{
	...
	public BatterPanel(InsertView insertView) {
		...
		setLayout(null);
		// 텍스트필드의 이름.
		String labelName[] = {"이름","나이","신장","타수", "안타수", "타율"};
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == btn[0]) {
			...
			String batCount = textField[3].getText();	
			String hit = textField[4].getText();
			String hitAvg = textField[5].getText();
			// 변수의 저장한 데이터를 기반으로 객체 생성.
			h = new Batter(0, name, Integer.parseInt(age), 
					Double.parseDouble(height), 
					Integer.parseInt(batCount),Integer.parseInt(hit),
					Double.parseDouble(hitAvg));
			// 추가가 성공적으로 됐는지 확인.
